...
async function main(env, { postgres, grpcLib }) {
	...
	async function saveDataToDB(sql, tableName, data, keys, returnIds) {
		try {
			const insertedRecords = await sql`INSERT INTO ${sql(tableName)} ${sql(
				data,
				...keys
			)} returning id`;
	...
	for (let table of tablesNames) {
		...
		// Get data by ids
		let { result: resultRecords } = await exploratoryDeploy({
			term: getRecordInDB(table, recordKeys),
		});
		const tableDataList =
			resultRecords.postblockdataList[0]?.exprsList[0]?.eTupleBody.psList;

...
import protoSchema from "../rnode-grpc-gen/js/pbjs_generated.json" assert { type: "json" };
...
export function rnodeService(env, grpcLib) {
	...
	if (!VALIDATOR_BOOT_PRIVATE)
		throw Error(
			`Environment parameter not set VALIDATOR_BOOT_PRIVATE, check .env file.`
		);

	// Validator (boot) node
	const deployService = rnodeDeploy({
		grpcLib,
		host: `${MY_NET_IP}:40401`,
		protoSchema,
	});
	const proposeService = rnodePropose({
		grpcLib,
		host: `${MY_NET_IP}:40402`,
		protoSchema,
	});
	...
	// Read-only node
	const deployServiceRead = rnodeDeploy({
		grpcLib,
		host: `${MY_NET_IP}:40411`,
		protoSchema,
	});
	...
	const exploratoryDeploy = makeExploratoryDeploy({
		deployService: deployServiceRead,
	});
...
function makeSendDeploy({ deployService, secretKey, phloLimit }) {
	...
	return async ({ term }) => {
		...
		] = await deployService.getBlocks({ depth: 1 });
		...
		const deployData = {
			...
			timestamp: Date.now(), // TODO: ambient access, move to input parameter 并发可能会使这里相同导致崩溃?
		};
		// Sign deploy
		const signed = signDeploy(secretKey, deployData);
		console.log({ timestamp: signed.timestamp, term: term.slice(50, 100) });

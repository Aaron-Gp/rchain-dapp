...
from zerver.models import PreregistrationUser, Realm, Stream, UserProfile, Extension
...
def home_real(request: HttpRequest) -> HttpResponse:
    ...
    if banned_desktop_app and False:
        return render(
            request,
    ...
    rev_address = user_profile.rev_address
    public_key = user_profile.public_key

    show_generate_wallet = False
    if rev_address is None:
        show_generate_wallet = True
    ...
    response = render(
        ...
        context={
            ...
            "show_generate_wallet": show_generate_wallet,
        },
    )
...
@zulip_login_required
def generate_wallet(request: HttpRequest) -> HttpResponse:
    return render(request, "zerver/generate_wallet.html")

@zulip_login_required
def add_wallet(request: HttpRequest) -> HttpResponse:
    user: UserProfile = request.user
    if request.method == 'GET':
        user.rev_address = request.GET.get("rev_address")
        user.public_key = request.GET.get("public_key")
        user.save()
    return HttpResponseRedirect(reverse("home"))
...
from confirmation.models import RealmCreationKey
...
class Command(ZulipBaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> str:
        ...
        if mode.lower() == "c":
            ...
            self.stdout.write(
                RealmCreationKey.objects.all().__str__().replace(">,", ">,\n")
            )
        elif mode.lower() == "d":
            ids = options["id"]
            if options["all"]:
                ...
                RealmCreationKey.objects.all().delete()
                self.stdout.write(
                    RealmCreationKey.objects.all().__str__()
                )
            else:
                for id in ids:
                    try:
                        prereguser = RealmCreationKey.objects.filter(id=id)
                        self.stdout.write(
                            self.style.WARNING(

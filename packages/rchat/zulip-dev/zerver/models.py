...
class Realm(models.Model):
    ...
    send_welcome_emails: bool = models.BooleanField(default=False)
    message_content_allowed_in_email_notifications: bool = models.BooleanField(default=False)
    ...
    def __str__(self) -> str:
        return f"[{self.id}]|{self.string_id}|{self.name}|{self.date_created}|{self.subdomain}>"
...
class RealmDomain(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.id}|{self.realm}|{self.domain}"
...
class Extension(models.Model):
    id: int = models.AutoField(auto_created=True, primary_key=True, verbose_name="ID")
    name: str = models.CharField(max_length=256, db_index=True, unique=True)
    link: str = models.CharField(max_length=256, null=False, blank=False)
...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    rev_address: Optional[str] = models.CharField(max_length=56, null=True, blank=False)
    public_key: Optional[str] = models.CharField(max_length=256, null=True, blank=False)
...
class PreregistrationUser(models.Model):
    ...
    def __str__(self) -> str:
        return f"[{self.id}] {self.email} {self.full_name} {self.status}"

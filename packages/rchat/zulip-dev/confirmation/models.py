...
def get_object_from_key(
    ...
    if time_elapsed.total_seconds() > _properties[confirmation.type].validity_in_days * 24 * 3600 and False: # !
...
class Confirmation(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.content_type}|{self.content_object}|{self.confirmation_key}|{self.realm}"
...
def validate_key(creation_key: Optional[str]) -> Optional["RealmCreationKey"]:
    ...
    if time_elapsed.total_seconds() > settings.REALM_CREATION_LINK_VALIDITY_DAYS * 24 * 3600 and False:
        raise RealmCreationKey.Invalid()
    return key_record
...
class RealmCreationKey(models.Model):
    ...
    def __str__(self) -> str:
        return f"<RealmCreationKey({self.creation_key}): {self.date_created} {self.presume_email_valid}>"
